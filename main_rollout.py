...
H1 = torch.cat(([H1.unsqueeze(2).unsqueeze(3)]*5),dim=2).float()
H2 = torch.cat(([H2.unsqueeze(2).unsqueeze(3)]*6),dim=2).float()
...
model = GNN_maker_HNN(graph1,6,64,6,["tanh",""],type="PULL")
print(model)
model2 = rollout_GNN_GRU(graph1,6,128,32,["tanh"," "],bias=False,type = PULL)
...
torch.save(model, stamp+"node5_nbody_model_rollout"+name+".pth")
...
gs=[]
for i in range(TIMESIZE*BATCHSIZE):
    src, dst = make_graph_no_loops(6,0)
    gtemp = dgl.graph((src,dst))
    #print(g.num_nodes())
    gs.append(gtemp)
#print(len(gs))
...
torch.save(model, stamp+"node6_nbody_model_rollout"+name+".pth")
